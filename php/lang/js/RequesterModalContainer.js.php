...
return [
    'regex' => [
        ...
        '/`\$\{([a-zA-Z\d_\.]+)\} is already your team member`/s' => '`${$1} 已经是您的团队成员`',
        '/`Current Version: v\$\{([a-zA-Z\d_\.]+)\}`/s' => '`当前版本: v${$1}`',
        '/`Save as a new \$\{([a-zA-Z\d_\.]+)\}`/s' => '`另存为新的${$1}`',
        '/`You\'re on Postman v\$\{([a-zA-Z\d_\.]+)\}`/s' => '`您使用的是 Postman v${$1}`',
        '/`Delete "\$\{([a-zA-Z\d_\.]+)\}"\?`/s' => '`删除 "${$1}"?`',
        '/`Postman v\$\{([a-zA-Z\d_\.]+)\} is the latest version\.`/s' => '`Postman v${$1} 是最新版本.`',
        '/`Connected to \$\{([a-zA-Z\d_\.]+)\}`/s' => '`已连接到 ${$1}`',
        '/`Failed to delete cookies for domain \$\{([a-zA-Z\d_\.]+)\}\.`/s' => '`无法删除 ${$1} 域的 Cookie.`',
        '/`Cookies for domain \$\{([a-zA-Z\d_\.]+)\} deleted successfully\.`/s' => '`${$1} 域的 Cookie 已成功删除.`',
        '/`\$\{([a-zA-Z\d_\.]+)\} shared with invited users`/s' => '`${$1} 与受邀请的用户共享`',
        '/`\$\{([a-zA-Z\d_\.]+)\} in Postman`/s' => '`${$1} 在 Postman`',
        '/`\$\{([a-zA-Z\d_\.]+)\} \$\{([a-zA-Z\d_\.]+)\} imported\.`/s' => '`${$1} ${$2} 已导入.`',
        '/`\$\{([a-zA-Z\d_\.]+)\} your collections`/s' => '`${$1}你的集合`',
        '/`Moving here will make the \$\{([\da-zA-Z_\(\)\-\+\[\]\.,":&\|\s]+)\} public`/s' => '`移到这里将使${({collection:"集合",environment:"环境"})[$1]||$1}公开`',
        '/`Invite people to \$\{([\da-zA-Z_\(\)\-\+\[\]\.,":&\|\s\?<>=]+)\} team`/s' => '`邀请人们加入 ${$1} 团队`',
        '/`\$\{([a-zA-Z\d_\.]+)\} emails are`/s' => '`${$1} 个邮箱`',
        '/`\$\{([a-zA-Z\d_\.]+)\} email is`/s' => '`${$1} 个邮箱`',
        '/`You updated \$\{([\da-zA-Z_\(\)\-\+\[\]\.,":&\|\s\?<>=]+)\}\.`/s' => '`您已更新 ${$1}.`',
    ...
    [
        ...
        '"You’re using an older version of Postman Desktop Agent, which can not be used to add or edit cookies. \n\nUpdate the Desktop Agent by clicking on the Postman Agent icon in your toolbar or switch to use the Postman Cloud Agent."' => '"您使用的是 Postman 桌面代理的旧版本, 该版本不能用于添加或编辑 Cookie. \n\n通过单击工具栏中的 Postman 代理图标更新桌面代理或切换到使用 Postman 云代理."',
        '"Disable Sync"' => '"禁用同步"',
        '"Disable syncing initial values of all the variables?"' => '"禁用同步所有变量的初始值?"',
        '"Disable syncing initial values of the secret variables?"' => '"禁用同步秘密变量的初始值?"',
        '"Enable syncing initial values of all the variables?"' => '"启用同步所有变量的初始值?"',
        '"Enable syncing initial values of the secret variables?"' => '"启用同步秘密变量的初始值?"',
        '"We will immediately stop syncing initial values of the variables with the Postman database and team members."' => '"我们将立即停止将变量的初始值与 Postman 数据库和团队成员同步."',
        '"Mock servers, monitors, and Newman using the initial values will be affected."' => '"使用初始值的模拟服务器、监视器和 Newman 将受到影响."',
        '"We will immediately start syncing the initial values of the variables with the Postman database and team members."' => '"我们将立即开始将变量的初始值与 Postman 数据库和团队成员同步."',
        '"Enter workspace name to confirm"' => '"输入工作区名称以确认"',
        '"Delete Workspace?"' => '"删除工作区?"',
        '"Everything in "' => '"所有在 "',
        '" workspace will be gone forever. Only collections can be restored from trash."' => '" 工作区中的内容都将永远消失. 只有集合能从回收站中恢复."',
        '" invalid."' => '"无效."',
        '"Download Win-64 bit App"' => '"下载 Win-64 位应用程序"',
        '"Remind Me Later"' => '"稍后提醒我"',
        '"Ending 32-bit app support"' => '"结束 32 位应用程序支持"',
        '"At Postman, we are ending support and maintenance for our 32-bit version of the app. You\'ll no longer receive feature, security, or other updates. We recommend you to switch to the 64-bit version."' => '"在 Postman, 我们将结束对 32 位版本应用程序的支持和维护. 您将不再收到功能、安全或其他更新. 我们建议您切换到 64 位版本."',
        ...
        '"An error occurred while editing the API version"' => '"编辑 API 版本时出错"',
        ...
        '"Delete API version?"' => '"删除 API 版本?"',
        '"An error occurred while deleting the API version"' => '"删除 API 版本时出错"',
        '"______"' => '"______"',
        '"______"' => '"______"',
        '"______"' => '"______"',
        '"______"' => '"______"',
