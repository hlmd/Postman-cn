...
return [
    'regex' => [
        ...
        '/`API Deployed on \$\{([a-zA-Z\d_\.]+)\}`/s' => '`API 部署于 ${$1}`',
        '/`Pull changes from \$\{([a-zA-Z\d_\.]+)\}`/s' => '`拉取变更从 ${$1}`',
        '/`Please enter a valid \$\{([a-zA-Z\d_\.]+)\} enterprise url`/s' => '`请输入一个有效的 ${$1} 企业网址`',
        '/`Commit and push changes to \$\{([a-zA-Z\d_\.]+)\}`/s' => '`提交并推送更改到 ${$1}`',
        '/`Select \$\{([\da-zA-Z_\(\)\[\]\.,"\|]+)\}`/s' => '`选择 ${$1}`',
        '/`This branch is currently used by version \$\{([a-zA-Z\d_\.]+)\}\. Using this branch will block the integration on \$\{([a-zA-Z\d_\.]+)\}\.`/s' => '`此分支当前由版本 ${$1} 使用. 使用此分支将阻止 ${$2} 上的集成.`',
        '/`All APIs \(\$\{([a-zA-Z\d_\.]+)\}\)`/s' => '`所有 API (${$1})`',
        '/`The API you\'re using no longer exists \(\$\{([a-zA-Z\d_\.]+)\}, \$\{([a-zA-Z\d_\.]+)\}\)\. Select another API\.`/s' => '`您正在使用的 API 不再存在 (${$1}, ${$2}). 选择另一个 API.`',
        '/`message \$\{([a-zA-Z\d_\.]+)\}`/s' => '`消息 ${$1}`',
        '/______/s' => '______',
        '/______/s' => '______',
        '/______/s' => '______',
    ],
    ...
    [
        ...
        '"Connect your API Gateway to view deployment status and deploy APIs  directly from Postman."' => '"连接您的 API 网关以查看部署状态并直接从 Postman 部署 API."',
        '"Deploy API"' => '"部署 API"',
        '"Deploy or Export to AWS Gateway"' => '"部署或导出到 AWS 网关"',
        '"You can deploy or export an API definition from a version or release to this AWS API Gateway."' => '"您可以将 API 定义从版本或发行版部署或导出到此 AWS API 网关."',
        '"Stages"' => '"阶段"',
        '"stages are not available in this gateway."' => '"阶段在此网关中不可用."',
        '"Stage"' => '"阶段"',
        '"Active Deployment"' => '"主动部署"',
        '"Exports from Postman"' => '"导出 从 Postman"',
        '"There has been no export of API Schema from Postman to AWS."' => '"没有将 API Schema 从 Postman 导出到 AWS."',
        '"Deployment Description"' => '"部署说明"',
        '"Deployment Source from Postman"' => '"来自 Postman 的部署源"',
        '"Successfully Deployed API"' => '"成功部署 API"',
        '"Successfully exported"' => '"成功导出"',
        '"We could not find the schema you are looking for."' => '"我们找不到您要查找的架构."',
        '"Deployment failed"' => '"部署失败"',
        '"Error occurred while deploying."' => '"部署时出错."',
        '"AWS Gateway error"' => '"AWS 网关错误"',
        '"Error occurred while fetching stages"' => '"提取阶段时发生错误"',
        '"Export and deploy API Schema on AWS"' => '"在 AWS 上导出和部署 API 架构"',
        '"Deploying an API from Postman will export the schema to the gateway and trigger deployment on the selected stage"' => '"从 Postman 部署 API 会将架构导出到网关并在所选阶段触发部署"',
        '"Deploy on a stage"' => '"部署到一个阶段"',
        '"This will push the schema from Postman to AWS onto a stage."' => '"这会将架构从 Postman 推送到 AWS 到一个阶段上."',
        '"The deployment will be successful only if at least one integration is configured over any API route in AWS"' => '"仅当在 AWS 中的任何 API 路由上配置了至少一个集成时, 部署才会成功"',
        '"AWS Stage"' => '"AWS 阶段"',
        '"Enter description"' => '"输入描述"',
        '"No projects found in your CircleCI account"' => '"在您的 CircleCI 帐户中未找到任何项目"',
        '"No projects found in your Travis CI account"' => '"在您的 Travis CI 帐户中未找到任何项目"',
        ...
        '"Uncommitted changes found"' => '"发现未提交的更改"',
        '"Commit and Push changes"' => '"提交和推送更改"',
        '"Commit and push them before creating a release."' => '"在创建发行之前提交并推送它们."',
        '"You don\'t have permission to view this release"' => '"您没有查看此发行的权限"',
        '"  to view this release."' => '"以查看此发行."',
        '"Request access"' => '"请求访问"',
        '"You do not have permission"' => '"您没有权限"',
        '"to view this version."' => '"以查看此版本."',
        '"You don\'t have permission to view this API version"' => '"您没有查看此 API 版本的权限"',
        '" scope(s) while creating an access token."' => '" 范围在创建访问令牌时"',
        '"Looks like you did not enter the access token associated with this custom domain. Please check the custom domain and the access token and try again."' => '"看起来您没有输入与此自定义域关联的访问令牌. 请检查自定义域和访问令牌, 然后再试一次."',
        '"Authentication Failed"' => '"授权失败"',
        '"Integrate a Git repository to this API to sync changes between them. "' => '"将 Git 存储库集成到此 API 以在它们之间同步更改. "',
        '"Your schema sync integration will be deleted"' => '"您的架构同步集成将被删除"',
        '"Connecting repository will delete the "' => '"连接存储库将删除"',
        '"schema sync integration"' => '"架构同步集成"',
        '" configured on this API Version"' => '"配置在此 API 版本上"',
        '"This version is already connected to a repository"' => '"此版本已连接到存储库"',
        ...
        '"Something went wrong while fetching the Home Page List"' => '"获取主页列表时出错"',
        '"Explore the workspaces where all the action is happening"' => '"探索所有行动发生的工作区"',
        '"API specifications with all the consumption resources set up"' => '"设置了所有消耗资源的 API 规范"',
        '"Ready collections to start working with"' => '"准备好开始使用的集合"',
        '"Meet Developers and Teams publishing APIs | Postman API Network"' => '"认识发布 API 的开发人员和团队 | Postman API 网络"',
        '"Related categories"' => '"相关类别"',
        '"Page Not Found"' => '"页面未找到"',
        '"Curated lists by the Postman team"' => '"Postman 团队的精选列表"',
        '"Categorised content to help you reach exactly what you need"' => '"分类内容可帮助您准确满足您的需求"',
        '"Discover high quality APIs in your domain of interest"' => '"在您感兴趣的领域发现高质量的 API"',
        '"All categories"' => '"所有类别"',
        '"Explore by category to discover high quality APIs in your domain of interest"' => '"按类别探索以发现您感兴趣的领域中的高质量 API"',
        '"Not sure which category to look within?"' => '"不确定要查看哪个类别?"',
        '"Explore what is in the spotlight"' => '"探索什么在精选中"',
        '"Something went wrong while fetching the APIs"' => '"获取 API 时出现问题"',
        '"Something went wrong while fetching the versions"' => '"获取版本时出现问题"',
        '"Something went wrong while creating the folders"' => '"创建文件夹时出现问题"',
        '"Something went wrong while deleting the folders"' => '"删除文件夹时出现问题"',
        '"Something went wrong while updating the folders"' => '"更新文件夹时出现问题"',
        '"Already added"' => '"已经添加"',
        '"You don’t have permission to add this API to the Private API Network."' => '"您无权将此 API 添加到私有 API 网络."',
        '"Search folders"' => '"搜索文件夹"',
        '"Organize your APIs in folders"' => '"在文件夹中组织您的 API"',
        '"Something went wrong while publishing the apis"' => '"发布 API 时出现问题"',
        '"Only versions of this API that are visible to consumers will be discoverable on the Private API Network"' => '"只有消费者可见的 API 版本才能在私有 API 网络上被发现"',
        '"Create and add to folder"' => '"创建并添加到文件夹"',
        '"API listing updated"' => '"API 列表已更新"',
        '"Something went wrong while updating folder"' => '"更新文件夹时出现问题"',
        '"Add APIs to Private API Network"' => '"将 API 添加到私有 API 网络"',
        '"Select APIs"' => '"选择 API"',
        '"Select folder"' => '"选择文件夹"',
        '"Add API"' => '"添加 API"',
        ...
        '" (you)"' => '" (你)"',
        '"Enter a folder name"' => '"输入一个文件夹名称"',
        '"Remove "' => '"移除"',
        '"All selected options have been cleared."' => '"所有选定的选项均已清除."',
        '"Enter a server URL before taking this action."' => '"在执行此操作之前输入服务器 URL."',
        '"Select a method before taking this action."' => '"在执行此操作之前选择一种方法."',
        '"The protobuf definition you selected contains no methods."' => '"您选择的 protobuf 定义不包含任何方法."',
        '"Choose a way to load services and methods before taking this action."' => '"在执行此操作之前选择一种加载服务和方法的方式."',
        '"Import failed. Try again with a valid URL."' => '"导入失败. 使用有效的 URL 重试."',
        '"There was an error while loading the protobuf definition from the API. Go through the details to know more."' => '"从 API 加载 protobuf 定义时出错. 浏览详细信息以了解更多信息."',
        '"There was an error while importing the protobuf definition. Go through the details to know more."' => '"导入 protobuf 定义时出错. 浏览详细信息以了解更多信息."',
        '"Couldn’t load the services and methods"' => '"无法加载服务和方法"',
        '"There was an error while loading the services and methods using server reflection. Go through the details to know more."' => '"使用服务器反射加载服务和方法时出错. 浏览详细信息以了解更多信息."',
        '"There was an error while loading the services and methods using server reflection."' => '"使用服务器反射加载服务和方法时出错."',
        '"Server reflection not supported"' => '"不支持服务器反射"',
        '"The server doesn’t support loading of services and methods using server reflection. Make sure you are using the correct URL."' => '"服务器不支持使用服务器反射加载服务和方法. 确保您使用的是正确的 URL."',
        '"There was an error while loading the services and methods using server reflection. Make sure you are using the correct URL and have the certificates configured (if required)."' => '"使用服务器反射加载服务和方法时出错. 确保您使用正确的 URL 并配置了证书 (如果需要)."',
        '"Invalid URL"' => '"无效的 URL"',
        '"The URL you have entered is invalid. Enter the correct URL and try again."' => '"您输入的网址无效. 输入正确的 URL, 然后重试."',
        '"Choose a way to load services and methods"' => '"选择一个加载服务和方法的方式"',
        '"Using server reflection"' => '"使用服务器反射"',
        '" or use any of the options below to load services and methods."' => '"或使用以下任何选项来加载服务和方法."',
        '"Load services and methods"' => '"加载服务和方法"',
        '"Choose how you want to load the services and methods. You can use a protobuf definition or load them using server reflection."' => '"选择您希望如何加载服务和方法. 您可以使用 protobuf 定义或使用服务器反射加载它们."',
        '"Use server reflection"' => '"使用服务器反射"',
        '"Enter server URL to take this action."' => '"输入服务器 URL 以执行此操作."',
        '"Import protobuf definition from local file"' => '"从本地文件导入 protobuf 定义"',
        '"Import protobuf definition from URL"' => '"从 URL 导入 protobuf 定义"',
        '"Create a new protobuf definition"' => '"创建一个新的 protobuf 定义"',
        '"More ways to load services and methods"' => '"更多加载服务和方法的方式"',
        '"A string of base64, or an array of numbers between 0 and 255"' => '"base64 字符串, 或 0 到 255 之间的数字数组"',
        '"An integer between -2147483648 and 2147483647"' => '"-2147483648 到 2147483647 之间的整数"',
        '"An integer between 0 and 4294967295"' => '"0 到 4294967295 之间的整数"',
        '"An integer between 0 and 18446744073709551615"' => '"0 到 18446744073709551615 之间的整数"',
        '"An integer between -9223372036854775808 and 9223372036854775807"' => '"-9223372036854775808 到 9223372036854775807 之间的整数"',
        '"(empty)"' => '"(空)"',
        '"Unknown field"' => '"未知的字段"',
        '"You cannot send authorization credentials over an insecure connection. Enable TLS by adding a certificate."' => '"您不能通过不安全的连接发送授权凭据. 通过添加证书启用 TLS."',
        '"______"' => '"______"',
        '"______"' => '"______"',
        '"______"' => '"______"',
        '"______"' => '"______"',
