...
return [
    ...
    "'This means that the server has received the request headers, and that the client should proceed to send the request body (in the case of a request for which a body needs to be sent; for example, a POST request). If the request body is large, sending it to a server when a request has already been rejected based upon inappropriate headers is inefficient. To have a server check if the request could be accepted based on the request\\'s headers alone, a client must send Expect: 100-continue as a header in its initial request and check if a 100 Continue status code is received in response before continuing (or receive 417 Expectation Failed and not continue).'" =>
        "'这意味着服务器已经收到了请求头,并且客户端应继续发送请求正文(在需要发送正文的请求的情况下;例如POST请求). 如果请求正文很大,则在基于不适当的头而已拒绝请求时将其发送到服务器效率不高. 若要让服务器仅根据请求的标头检查是否可以接受该请求,客户端必须在其初始请求中发送Expect: 100-continue作为头,并检查是否在响应中收到100继续状态码,然后继续(或收到417预期失败,因此无法继续).'",

    // 101
    "'Switching Protocols'" => "'切换协议'",
    "'This means the requester has asked the server to switch protocols and the server is acknowledging that it will do so.'" =>
        "'这意味着请求者已请求服务器切换协议,并且服务器已确认将这样做.'",

    // 102
    "'Processing'" => "'处理中'",
    "'Processing (WebDAV) (RFC 2518)'" => "'处理中(WebDAV) (RFC 2518)'",
    "'As a WebDAV request may contain many sub-requests involving file operations, it may take a long time to complete the request. This code indicates that the server has received and is processing the request, but no response is available yet. This prevents the client from timing out and assuming the request was lost.'" =>
        "'由于WebDAV请求可能包含许多涉及文件操作的子请求,因此完成请求可能需要很长时间. 该代码表示服务器已接收并正在处理请求,但是尚无响应. 这样可以防止客户端超时并假设请求已丢失.'",

    // 103
    "'Checkpoint'" => "'检查点'",
    "'This code is used in the Resumable HTTP Requests Proposal to resume aborted PUT or POST requests.'" =>
        "'此代码用于可恢复HTTP请求建议,以恢复中止的PUT或POST请求.'",
    ...
    "'This is a non-standard IE7-only code which means the URI is longer than a maximum of 2083 characters.'" =>
        "'这是非标准的仅限IE7的代码,这意味着URI的长度超过了2083个字符的最大长度.'",

    // 200
    "'OK'" => "'OK'",
    "'Standard response for successful HTTP requests. The actual response will depend on the request method used. In a GET request, the response will contain an entity corresponding to the requested resource. In a POST request the response will contain an entity describing or containing the result of the action.'" =>
        "'成功HTTP请求的标准响应. 实际响应将取决于所使用的请求方法. 在GET请求中,响应将包含与所请求资源相对应的实体. 在POST请求中,响应将包含描述或包含操作结果的实体.'",
    ...
    "'The request has been accepted for processing, but the processing has not been completed. The request might or might not eventually be acted upon, as it might be disallowed when processing actually takes place.'" =>
        "'该请求已被接受进行处理,但是处理尚未完成. 该请求最终可能会执行,也可能不会执行,因为在实际进行处理时,请求可能被禁止.'",
    ...
    "'Indicates multiple options for the resource that the client may follow. It, for instance, could be used to present different format options for video, list files with different extensions, or word sense disambiguation.'" =>
        "'指示客户端可以遵循的资源的多个选项. 例如,它可以用于呈现视频的不同格式选项,具有不同扩展名的列表文件或消除词义歧义.'",

    // 301
    "'Moved Permanently'" => "'永久移动到'",
    "'This and all future requests should be directed to the given URI.'" =>
        "'此请求和所有将来的请求都应定向到给定的URI.'",
    ...
    "'This is an example of industrial practice contradicting the standard. HTTP/1.0 specification (RFC 1945) required the client to perform a temporary redirect (the original describing phrase was \"Moved Temporarily\"), but popular browsers implemented 302 with the functionality of a 303. Therefore, HTTP/1.1 added status codes 303 and 307 to distinguish between the two behaviours. However, some Web applications and frameworks use the 302 status code as if it were the 303.'" =>
        "'这是与标准相矛盾的工业实践的一个例子. HTTP/1.0规范(RFC 1945)要求客户端执行临时重定向(原始描述短语为\"临时移动\"),但是流行的浏览器使用303的功能实现了302. 因此,HTTP/1.1添加了状态码303和307来区分这两种行为. 但是,某些Web应用程序和框架使用302状态码就像使用303.'",

    // 303
    "'See Other'" => "'看其他'",
    "'The response to the request can be found under another URI using a GET method. When received in response to a POST (or PUT/DELETE), it should be assumed that the server has received the data and the redirect should be issued with a separate GET message.'" =>
        "'可以使用GET方法在另一个URI下找到对请求的响应. 当收到响应POST(或PUT/DELETE)的响应时,应假定服务器已接收到数据,并且应该使用单独的GET消息发布重定向.'",

    // 304
    "'Not Modified'" => "'未修改'",
    "'Indicates the resource has not been modified since last requested. Typically, the HTTP client provides a header like the If-Modified-Since header to provide a time against which to compare. Using this saves bandwidth and reprocessing on both the server and client, as only the header data must be sent and received in comparison to the entirety of the page being re-processed by the server, then sent again using more bandwidth of the server and client.'" =>
        "'指示自上次请求以来尚未修改资源. 通常,HTTP客户端提供类似于If-Modified-Since头的头,以提供比较的时间. 使用此功能可以节省带宽和在服务器和客户端上进行重新处理,因为与服务器正在重新处理的整个页面相比,只发送和接收头数据,然后使用服务器和客户端上的更多带宽再次发送.'",

    // 305
    "'Use Proxy'" => "'使用代理'",
    "'Use Proxy (since HTTP/1.1)'" => "'使用代理(since HTTP/1.1)'",
    "'Many HTTP clients (such as Mozilla and Internet Explorer) do not correctly handle responses with this status code, primarily for security reasons.'" =>
        "'主要出于安全原因,许多HTTP客户端(例如Mozilla和Internet Explorer)不能正确处理带有此状态码的响应.'",

    // 306
    "'Switch Proxy'" => "'切换代理'",
    "'No longer used. Originally meant \"Subsequent requests should use the specified proxy.\"'" =>
        "'不再使用. 最初的意思是\"后续请求应使用指定的代理\"'",
    ...
    "'In this occasion, the request should be repeated with another URI, but future requests can still use the original URI. In contrast to 303, the request method should not be changed when reissuing the original request. For instance, a POST request must be repeated using another POST request.'" =>
        "'在这种情况下,应该使用另一个URI重复该请求,但是以后的请求仍可以使用原始URI. 与303相反,重新发出原始请求时,不应更改请求方法. 例如,必须使用另一个POST请求重复POST请求.'",
...
//        "'此代码用于可恢复HTTP请求建议,以恢复中止的PUT或POST请求.'",
    ...
    "'Similar to 403 Forbidden, but specifically for use when authentication is possible but has failed or not yet been provided. The response must include a WWW-Authenticate header field containing a challenge applicable to the requested resource.'" =>
        "'与403 Forbidden(禁止)相似,但专门用于可能进行身份验证但已失败或尚未提供身份验证的情况. 响应必须包含一个WWW-Authenticate头字段,其中包含适用于所请求资源的质询.'",
    ...
    "'Reserved for future use. The original intention was that this code might be used as part of some form of digital cash or micropayment scheme, but that has not happened, and this code is not usually used. As an example of its use, however, Apple\\'s MobileMe service generates a 402 error (\"httpStatusCode:402\" in the Mac OS X Console log) if the MobileMe account is delinquent.'" =>
        "'保留以备将来使用. 最初的意图是将该代码用作某种形式的数字现金或小额支付方案的一部分,但这并未发生,因此通常不使用此代码. 但是,作为其使用示例,如果MobileMe账户欠款,Apple的MobileMe服务会生成402错误(在Mac OS X控制台日志中为\"httpStatusCode:402\").'",

    // 403
    "'Forbidden'" => "'禁止'",
    "'The request was a legal request, but the server is refusing to respond to it. Unlike a 401 Unauthorized response, authenticating will make no difference.'" =>
        "'该请求是合法请求,但是服务器拒绝响应. 与401 Unauthorized(未经授权)的响应不同,身份验证没有任何区别.'",
    ...
    "'A request was made of a resource using a request method not supported by that resource; for example, using GET on a form which requires data to be presented via POST, or using PUT on a read-only resource.'" =>
        "'使用资源不支持的请求方法对资源进行了请求. 例如,在需要通过POST呈现数据的表单上使用GET,或在只读资源上使用PUT.'",

    // 406
    "'Not Acceptable'" => "'不接受'",
    "'The requested resource is only capable of generating content not acceptable according to the Accept headers sent in the request.'" =>
        "'根据请求中发送的Accept头,所请求的资源只能生成不可接受的内容.'",
    ...
    "'The server timed out waiting for the request. According to W3 HTTP specifications: \"The client did not produce a request within the time that the server was prepared to wait. The client MAY repeat the request without modifications at any later time.\"'" =>
        "'服务器超时,等待请求. 根据W3 HTTP规范: \"客户端在服务器准备等待的时间内没有产生请求. 客户端可以在以后的任何时间重复请求而无需进行修改.\"'",

    // 409
    "'Conflict'" => "'冲突'",
    "'Indicates that the request could not be processed because of conflict in the request, such as an edit conflict.'" =>
        "'表示由于请求中的冲突(例如编辑冲突)而无法处理该请求.'",

    // 410
    "'Gone'" => "'消失了'",
    "'Indicates that the resource requested is no longer available and will not be available again. This should be used when a resource has been intentionally removed and the resource should be purged. Upon receiving a 410 status code, the client should not request the resource again in the future. Clients such as search engines should remove the resource from their indices. Most use cases do not require clients and search engines to purge the resource, and a \"404 Not Found\" may be used instead.'" =>
        "'指示请求的资源不再可用,并且将不再可用. 当故意删除资源并清除该资源时,应使用此方法. 收到410状态码后,客户端以后不应再请求资源. 诸如搜索引擎之类的客户端应从其索引中删除资源. 大多数用例不需要客户端和搜索引擎清除资源,而是可以使用\"404 Not Found(找不到)\"代替.'",
    ...
    "'The client has asked for a portion of the file, but the server cannot supply that portion. For example, if the client asked for a part of the file that lies beyond the end of the file.'" =>
        "'客户端要求提供文件的一部分,但是服务器无法提供该部分. 例如,如果客户要求文件的一部分超出文件末尾.'",
    ...
    "'This code was defined in 1998 as one of the traditional IETF April Fools\\' jokes, in RFC 2324, Hyper Text Coffee Pot Control Protocol, and is not expected to be implemented by actual HTTP servers. However, known implementations do exist.'" =>
        "'该代码在1998年被定义为RFC 2324中的传统IETF愚人节笑话之一,超文本咖啡壶控制协议,并且预计不会由实际的HTTP服务器实现. 但是,确实存在已知的实现.'",
    ...
    "'The request failed due to failure of a previous request (e.g. a PROPPATCH).'" =>
        "'由于前一个请求(例如: 一个PROPPATCH)失败,因此请求失败.'",
    ...
    "'Defined in drafts of \"WebDAV Advanced Collections Protocol\",[14] but not present in \"Web Distributed Authoring and Versioning (WebDAV) Ordered Collections Protocol\".[15]'" =>
        "'在\"WebDAV高级集合协议\"[14]的草稿中定义,但在\"Web分布式创作和版本控制(WebDAV)有序集合协议\"[15]中不存在.'",
    ...
    "'The origin server requires the request to be conditional. Intended to prevent \"the \\'lost update\\' problem, where a client GETs a resource\\'s state, modifies it, and PUTs it back to the server, when meanwhile a third party has modified the state on the server, leading to a conflict.\"[17] Proposed in an Internet-Draft.'" =>
        "'原始服务器要求该请求是有条件的. 旨在防止\"丢失更新\"问题,即当第三方修改了服务器上的状态,从而导致冲突时,客户端获取资源的状态,将其修改,然后将其放回服务器. [17]在互联网草案中提出.'",
    ...
    "'The user has sent too many requests in a given amount of time. Intended for use with rate limiting schemes. Proposed in an Internet-Draft.'" =>
        "'用户在给定的时间内发送了太多请求. 用于速率限制方案. 在互联网草案中提出.'",
    ...
    "'The server is unwilling to process the request because either an individual header field, or all the header fields collectively, are too large. Proposed in an Internet-Draft.'" =>
        "'该服务器不愿意处理该请求,因为单个头字段或所有头字段加起来都太大了. 在互联网草案中提出.'",
    ...
    "'An nginx HTTP server extension. The server returns no information to the client and closes the connection (useful as a deterrent for malware).'" =>
        "'一个Nginx HTTP服务器扩展. 服务器不向客户端返回任何信息,并关闭连接(用于阻止恶意软件).'",

    // 449
    "'Retry With'" => "'重试随着'",
    "'A Microsoft extension. The request should be retried after performing the appropriate action.'" =>
        "'一个Microsoft扩展. 执行适当的操作后.应重试该请求.'",

    // 450
    "'Blocked by Windows Parental Controls'" => "'被Windows家长控制阻止'",
    "'A Microsoft extension. This error is given when Windows Parental Controls are turned on and are blocking access to the given webpage.'" =>
        "'一个Microsoft扩展. 打开Windows家长控制并阻止访问给定网页时,会出现此错误.'",
    ...
    "'The client needs to authenticate to gain network access. Intended for use by intercepting proxies used to control access to the network (e.g. \"captive portals\" used to require agreement to Terms of Service before granting full Internet access via a Wi-Fi hotspot). Proposed in an Internet-Draft.'" =>
        "'客户端需要进行身份验证才能获得网络访问权限. 意图通过拦截代理来控制对网络的访问(例如: 用于在通过Wi-Fi热点授予完全互联网访问权限之前要求同意服务条款的\"强制门户\"). 在互联网草案中提出.'",
    ...
    "'This status code is not specified in any RFCs, but is used by some HTTP proxies to signal a network read timeout behind the proxy to a client in front of the proxy.'" =>
        "'此状态码未在任何RFC中指定,但是某些HTTP代理使用此状态码向代理之前的客户端发送代理后的网络读取超时信号'",

    // 599
    "'Network connect timeout error[23]'" => "'网络连接超时错误[23]'",
    "'This status code is not specified in any RFCs, but is used by some HTTP proxies to signal a network connect timeout behind the proxy to a client in front of the proxy.'" =>
        "'此状态码未在任何RFC中指定,但是某些HTTP代理使用此状态码向代理之前的客户端发送代理后的网络连接超时信号.'",
